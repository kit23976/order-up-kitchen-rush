...
{
    Debug.Log("UpdatePlayerScore called with score: " + score);  // ตรวจสอบว่าโค้ดถูกเรียกหรือไม่
    _playerScore = score;  // อัพเดตค่าของ _playerScore
    _playerScoreText.text = $"Your score: {_playerScore}";  // อัพเดตข้อความที่แสดงบน UI
}
        ...
        public void Load()
        {
            ...
            Leaderboards.order.GetEntries(searchQuery, OnLeaderboardLoaded, ErrorCallback);
        ...
        public void Submit()
        {
            Leaderboards.order.UploadNewEntry(_playerUsernameInput.text, _playerScore, Callback, ErrorCallback);
        }
        
        public void DeleteEntry()
        {
            Leaderboards.order.DeleteEntry(Callback, ErrorCallback);
        }
        ...
        {
            Leaderboards.order.GetPersonalEntry(OnPersonalEntryLoaded, ErrorCallback);
        }
